...
export default class Promote extends React.Component {
  renderSquare = square => (
    <Square
      piece={square}
      style={square.style}
      onClick={this.props.promoteHandle.bind(this, square)}
    />
  );
  ...
  render() {
    ...
    return (
      <div
        className={"promote promote-" + promoteColor}
        id={"promote-" + this.props.id}
      >
        {this.promotePieces.map((ws, index) => this.renderSquare(ws, index))}
      </div>
    );
  }
}
